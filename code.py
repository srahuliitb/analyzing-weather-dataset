...
def clear(df,col,val):
    ...
    """
    value_counts = df[df[col] == val].shape[0]
    return value_counts
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    """
    aggregated_value = pd.pivot_table(df, index = pd.to_datetime(df[date_col]).dt.month, 
                                      values = agg_col, aggfunc = agg)
...
def convert(df,celsius):
    ...
    """
    converted_temp = 9 * df[celsius] / 5 + 32
...
print(agg_values_ina_month(weather, 'Date/Time', 'Temp (C)', 'mean'))
